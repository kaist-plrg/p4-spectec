...
A nested `struct` used as a field in a `header` must contain only `bit<W>`,
`int<W>`, a serializable `enum`, or a `bool`.
...
The symbol `type` in the grammar must be one of the following types:
