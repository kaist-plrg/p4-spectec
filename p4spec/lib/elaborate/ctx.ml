...
let find_plain_dec_opt (ctx : t) (fid : FId.t) :
  ...
  Option.bind func_opt (function
    | Func.Plain (tparams, params, plaintyp, clauses) ->
        Some (tparams, params, plaintyp, clauses)
    | Func.Table _ | Func.Extern _ | Func.Builtin _ -> None)

let find_plain_dec (ctx : t) (fid : FId.t) :
    tparam list * param list * plaintyp * Il.Ast.clause list =
  match find_plain_dec_opt ctx fid with
  | Some (tparams, params, plaintyp, clauses) ->
      (tparams, params, plaintyp, clauses)
  | None -> error_undef fid.at "plain function" fid.it

let bound_plain_dec (ctx : t) (fid : FId.t) : bool =
  find_plain_dec_opt ctx fid |> Option.is_some

let find_table_dec_opt (ctx : t) (fid : FId.t) :
    (param list * plaintyp * Il.Ast.tblrow list) option =
  let func_opt = FEnv.find_opt fid ctx.fenv in
  Option.bind func_opt (function
    | Func.Table (params, plaintyp, tblrows) -> Some (params, plaintyp, tblrows)
    | Func.Plain _ | Func.Extern _ | Func.Builtin _ -> None)
...
let find_dec_signature_opt (ctx : t) (fid : FId.t) :
  ...
  |> Option.map (function
       ...
       | Func.Plain (tparams, params, plaintyp, _) ->
...
let add_plain_dec (ctx : t) (fid : FId.t) (tparams : tparam list)
    (params : param list) (plaintyp : plaintyp) : t =
  if bound_dec ctx fid then error_dup fid.at "function" fid.it;
  let func = Func.Plain (tparams, params, plaintyp, []) in
  let fenv = FEnv.add fid func ctx.fenv in
  { ctx with fenv }

let add_table_def (ctx : t) (fid : FId.t) (tblrows : Il.Ast.tblrow list) : t =
  if not (bound_dec ctx fid) then
    error_undef (List.hd tblrows).at "table function" fid.it;
  let params, plaintyp, tblrows_found = find_table_dec ctx fid in
  if List.length tblrows_found > 0 then
    error_dup (List.hd tblrows_found).at "table function" fid.it;
  let func = Func.Table (params, plaintyp, tblrows) in
  let fenv = FEnv.add fid func ctx.fenv in
  { ctx with fenv }

let add_plain_clause (ctx : t) (fid : FId.t) (clause : Il.Ast.clause) : t =
  if not (bound_plain_dec ctx fid) then
    error_undef clause.at "plain function" fid.it;
  let tparams, params, plaintyp, clauses = find_plain_dec ctx fid in
  let func = Func.Plain (tparams, params, plaintyp, clauses @ [ clause ]) in
  let fenv = FEnv.add fid func ctx.fenv in
  { ctx with fenv }
