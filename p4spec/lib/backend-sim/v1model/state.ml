...
module ArchState = struct
  ...
  let with_resubmit_opt (resubmit_opt : Packet.ResubmitInfo.t option) (t : t) :
      t =
    { t with resubmit_opt }
  ...
  let with_mirror_tbl (mirror_tbl : MirrorTable.t) (t : t) =
    { t with mirror_tbl }
  ...
  let of_value (v : Value.t) =
    v |> unwrap_extern_v |> of_yojson |> Result.get_ok
end
