...
module Make
  ...
  let cover_instr_stfs (spec : spec) (includes_p4 : string list)
      (filenames_p4 : string list) (filenames_stf : string list list) : ICov_multi.t
      =
    verbose := false;
    Arch.init spec;
    let cover_multi =
    ...
    |> List.fold_left
         (fun cover_multi (filename_p4, filenames_stf) ->
           List.fold_left
             (fun cover_multi filename_stf ->
               let (module IH : Inst.Handler.HANDLER), read_coverage_instr =
                 Inst.Coverage_instr.make ()
               in
               Inst.Hook.register [ (module IH : Inst.Handler.HANDLER) ];
               ...
               let cover_single = read_coverage_instr () in
               ICov_multi.extend cover_multi filename_p4 cover_single)
             cover_multi filenames_stf)
         cover_multi

  let cover_dangling_stfs (spec : spec) (includes_p4 : string list)
      (filenames_p4 : string list) (filenames_stf : string list list) : DCov_multi.t
      =
    verbose := false;
    Arch.init spec;
    let cover_multi =
