  ...
  List.fold_left
    (fun (out_vars_acc, in_vars_acc) (iter, vars) ->
      ...
      let out_vars, in_vars =
        List.partition
          (fun (id, _, _) -> Domain.Lib.IdSet.mem id out_ids)
          vars
      in
...
let prosify_guard ctx exp_case guard : Pl.Ast.cond =
  let exp_of_guard : Pl.Ast.exp = 
    ...
    | BoolG b -> 
      if b then exp_case
      else Il.Ast.UnE (`NotOp, `BoolT, exp_case) $$ (exp_case.at, exp_case.note)
    | CmpG (cmpop, optyp, exp_r) ->
      Il.Ast.CmpE (cmpop, optyp, exp_case, exp_r) $$ (exp_case.at, exp_case.note)
    | SubG typ ->
      Il.Ast.SubE (exp_case, typ) $$ (exp_case.at, exp_case.note)
    | MatchG pattern ->
      Il.Ast.MatchE (exp_case, pattern) $$ (exp_case.at, exp_case.note)
    | MemG exp ->
      Il.Ast.MemE (exp_case, exp) $$ (exp_case.at, exp_case.note)
  in
...
let rec prosify_case ctx exp (guard, instrs) : Pl.Ast.instr list =
  ...
  | Some Check ->
    [ Pl.Ast.Check cond $ no_region ] @ instrs_pl
  | Some If -> 
    [ Pl.Ast.Branch (Pl.Ast.If, cond, instrs_pl) $ no_region ]
  | Some ElseIf -> 
    [ Pl.Ast.Branch (Pl.Ast.ElseIf, cond, instrs_pl) $ no_region ]
  | Some Else -> 
    [ Pl.Ast.Branch (Pl.Ast.Else, cond, instrs_pl) $ no_region ]
  | None -> assert false
...
and prosify_instr ctx instr : Pl.Ast.instr list =
  ...
  | IfI (exp, iterexps, instrs, phantom) -> (
    ...
    | Some cond_style -> (
      let branchtype = match cond_style with
      | If -> Pl.Ast.If
  ...
  | HoldI (id, (mixop, exps), iterexps, holdcase) -> (
    match holdcase with
      | BothH (instrs_hold, instrs_nothold) -> (
        ...
        let relation_true = match HEnv.get_rel id ctx.penv.prose_true with
          | Some hintexp -> Pl.Ast.Prose (hintexp, [], exps)
          | None -> Pl.Ast.Mixop (mixop, exps)
        in
        let cond_if = Pl.Ast.RelCond (relation_true, id) |> prosify_iterated_cond ~neg:false iterexps in
        let instr_if = Pl.Ast.Branch (Pl.Ast.If, cond_if, instrs_hold_sl) $ instr.at in
        (* create else-branch for not-hold *)
        let instrs_nothold_sl = prosify_instrs ctx instrs_nothold in
        let relation_false = match HEnv.get_rel id ctx.penv.prose_false with
          | Some hintexp -> Pl.Ast.Prose (hintexp, [], exps)
          | None -> Pl.Ast.Mixop (mixop, exps)
        in
        let cond_else = Pl.Ast.RelCond (relation_false, id) |> prosify_iterated_cond ~neg:true iterexps in
        let instr_else = Pl.Ast.Branch (Pl.Ast.Else, cond_else, instrs_nothold_sl) $ instr.at in
        [ instr_if; instr_else ]
    )
      | HoldH (instrs_hold, _) -> (
        let instrs_hold_sl = prosify_instrs ctx instrs_hold in
        let relation_true = match HEnv.get_rel id ctx.penv.prose_true with
          ...
          | None -> Pl.Ast.Mixop (mixop, exps)
        in
        let cond = Pl.Ast.RelCond (relation_true, id) |> prosify_iterated_cond iterexps in
        let instr = Pl.Ast.Branch (Pl.Ast.If, cond, instrs_hold_sl) $ instr.at in
        [ instr ]
    )
      | NotHoldH (instrs_nothold, _) -> (
        ...
        let relation_false = match HEnv.get_rel id ctx.penv.prose_false with
          ...
          | None -> Pl.Ast.Mixop (mixop, exps)
        in
        let cond = Pl.Ast.RelCond (relation_false, id) |> prosify_iterated_cond iterexps in
        let instr = Pl.Ast.Branch (Pl.Ast.If, cond, instrs_nothold_sl) $ instr.at in
        [ instr ]
      )
    )
  | CaseI (exp, cases, Some _) -> 
    prosify_cases ctx ~closed:false exp cases
  | CaseI (exp, cases, None) -> 
    prosify_cases ctx ~closed:true exp cases
  | OtherwiseI instr -> (
  ...
  | RuleI (id, (mixop, exps), iterexps) -> 
    ...
    let relation = match hint_opt with
      ...
      | None -> Pl.Ast.Mixop (mixop, exps)
    in
    [ Pl.Ast.Rel (relation, id) $ instr.at |> prosify_iterated_let exps_out iterexps ]
  | ResultI exps ->
...
and prosify_instrs ctx (instrs : instr list) : Pl.Ast.instr list =
  ...
  if num_if_instrs = 1 then
    instrs
    |> List.concat_map (prosify_instr (ctx |> as_cond Check))
  else
    List.mapi
      (fun i instr ->
