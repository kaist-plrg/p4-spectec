...
let rec sub_ (find_typdef_opt : TId.t -> Type.Typdef.t option)
    (find_func_opt : FId.t -> (tparam list * typ list * typ) option) (typ : typ)
    (value : value) : bool =
  match typ.it with
  ...
  | VarT (tid, targs) -> (
      ...
      | Defined (tparams, deftyp) -> (
          ...
          | PlainT typ, _ ->
              ...
              sub_ find_typdef_opt find_func_opt typ value
          ...
          | VariantT typcases, CaseV valuecase ->
              ...
              List.exists
                (fun (nottyp, _, _) ->
                  ...
                  subs_ find_typdef_opt find_func_opt typs values)
                typcases
          | _ -> false))
  | TupleT typs -> (
      match value.it with
      | TupleV values ->
          ...
          && List.for_all2 (sub_ find_typdef_opt find_func_opt) typs values
      | _ -> false)
  | IterT (typ_inner, Opt) -> (
      match value.it with
      | OptV value_opt -> (
          ...
          | Some value_inner ->
              sub_ find_typdef_opt find_func_opt typ_inner value_inner
          | None -> true)
      | _ -> true)
  | IterT (typ_inner, List) -> (
      match value.it with
      | ListV values ->
          List.for_all (sub_ find_typdef_opt find_func_opt typ_inner) values
      | _ -> false)
  | FuncT (tparams_t, typs_params_t, typ_ret_t) -> (
      match value.it with
      | FuncV fid -> (
          match find_func_opt fid with
          | Some (tparams_v, typs_params_v, typ_ret_v) ->
...
and subs_ (find_typdef_opt : TId.t -> Type.Typdef.t option)
    (find_func_opt : FId.t -> (tparam list * typ list * typ) option)
    (typs : typ list) (values : value list) : bool =
  List.length typs = List.length values
  && List.for_all2 (sub_ find_typdef_opt find_func_opt) typs values
...
let sub cache_sub_var find_typdef_opt find_func_opt typ value =
  match typ.it with
  | VarT (tid, []) -> (
      ...
      | None ->
          let res =
            sub_ (cache_find_typdef_opt find_typdef_opt) find_func_opt typ value
          in
          Hashtbl.add cache_sub_var key res;
          res)
  | _ -> sub_ (cache_find_typdef_opt find_typdef_opt) find_func_opt typ value

let subs find_typdef_opt find_func_opt typs values =
  subs_ (cache_find_typdef_opt find_typdef_opt) find_func_opt typs values

(* Entry point *)

let rec check cache_sub_var find_typdef_opt find_func_opt subcheck value =
  ...
  | TupleSC subchecks, TupleV values ->
      ...
      && List.for_all2
           (check cache_sub_var find_typdef_opt find_func_opt)
           subchecks values
  | IterSC (Opt, _), OptV None -> true
  | IterSC (Opt, subcheck), OptV (Some value) ->
      check cache_sub_var find_typdef_opt find_func_opt subcheck value
  | IterSC (List, subcheck), ListV values ->
      List.for_all
        (check cache_sub_var find_typdef_opt find_func_opt subcheck)
        values
  | RecurseSC typ, _ ->
      sub cache_sub_var find_typdef_opt find_func_opt typ value
  | _ -> false
