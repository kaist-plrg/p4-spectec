...
let rec sub_typ (find_typedef_opt : TId.t -> Typdef.t option) (typ_a : typ)
    (typ_b : typ) : bool =
  Equiv.equiv_typ find_typedef_opt typ_a typ_b
  || sub_typ' find_typedef_opt typ_a typ_b

and sub_typ' (find_typedef_opt : TId.t -> Typdef.t option) (typ_a : typ)
    (typ_b : typ) : bool =
  ...
  | VarT (tid_a, targs_a), VarT (tid_b, targs_b) -> (
      let td_opt_a = find_typedef_opt tid_a in
      let td_opt_b = find_typedef_opt tid_b in
      ...
      | ( Some (Defined (tparams_a, deftyp_a)),
          ...
          | VariantT typcases_a, VariantT typcases_b ->
              ...
              List.for_all
                (fun nottyp_a ->
                  List.exists
                    (Equiv.equiv_nottyp find_typedef_opt nottyp_a)
                    nottyps_b)
                nottyps_a
          | _, _ -> false)
      | _ -> false)
  | TupleT typs_a, TupleT typs_b ->
      ...
      && List.for_all2 (sub_typ find_typedef_opt) typs_a typs_b
  | IterT (typ_a, iter_a), IterT (typ_b, iter_b) when iter_a = iter_b ->
      sub_typ find_typedef_opt typ_a typ_b
  | IterT (typ_a, Opt), IterT (typ_b, List) ->
      sub_typ find_typedef_opt typ_a typ_b
  | _, IterT (typ_b, Opt) -> sub_typ find_typedef_opt typ_a typ_b
  | _, IterT (typ_b, List) -> sub_typ find_typedef_opt typ_a typ_b
  | _ -> false
