(* The value-representation contracts at the compiled-spec <-> extern boundary.
   ...
   Three signatures, layered by capability:
     - [SAFE]: the structural contract (marshal, Get, Make). The [builtin] and
       [interface] layers and the structural [backend-sim] functors are generic
       over [SAFE], so the same code runs against two value representations and
       none of them can reinterpret a value.
     - [UNSAFE]: the unchecked reinterpret across the runner edge (see below).
     - [VAL]: [SAFE] + [UNSAFE] — what a concrete representation provides.
...
module type SAFE = sig
  ...
  (* The value's source region, when the representation carries one.
     [V_value] returns the real region; [V_native] has none (compiled values
     carry no per-node location), so it returns [no_region]. *)
  val at : t -> region

  (* [marshal]/[unmarshal]: convert a value of (statically known) spec type
     [typ] to/from a real [Value.t] when it must be stored in a serialized
     field (scheduler [Packet.value_ctx], register payloads). Identity under
  ...
  module Get : sig
    ...
    (* [case]/[( |>>? )] take the value's spec type as an [Il.typ] and the
       mixop as an [Il.mixop]. [V_value] ignores the typ (each [Value.t] case
       carries its mixop tag); [V_native] needs it to pick the OCaml variant
       projection. [( |>>? )] takes [(mixop, typ)] as a pair so it stays
       usable infix. *)
...
(* [UNSAFE]: the unchecked reinterpret across the [Value.t]-typed dynamic-
   runner edge. [to_value]/[of_value] pass a value straight back to compiled
   code, where it is never decoded — identity under [V_value], a bare
   [Obj.obj]/[Obj.repr] under [V_native]. This is the dangerous escape hatch,
   so it is deliberately NOT part of [SAFE]: the generic [builtin] layer and
   the structural [backend-sim] functors only ever see [SAFE] and cannot cast.
...
(* [VAL]: what a concrete value representation actually provides — the [SAFE]
   contract plus the [UNSAFE] escape hatch. [V_value]/[V_native] satisfy it,
   and [Spec.V] is typed at it so the arch/pipe layer keeps cast access. *)
module type VAL = sig
  include SAFE
