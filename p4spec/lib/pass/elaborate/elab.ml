...
let as_text_typ (ctx : Ctx.t) (typ_il : Il.typ) : unit attempt =
  let typ_il = Types.Expand.expand_typ ctx.tdenv typ_il in
...
and infer_list_exp (ctx : Ctx.t) (at : region) (exps : exp list) :
  ...
  | exp :: exps ->
      let* ctx, exp_il, typ_il = infer_exp ctx exp in
      let* ctx, exps_il, typs_il = infer_exps ctx exps in
      if List.for_all (Types.Equiv.equiv_typ ctx.tdenv typ_il) typs_il then
...
and infer_call_exp (ctx : Ctx.t) (at : region) (id : id) (targs : targ list)
  ...
  let params_il = Typ.subst_params theta params_il in
  let typ_il = Typ.subst_typ theta typ_il in
...
and infer_sub_exp (ctx : Ctx.t) (exp : exp) (plaintyp : plaintyp) :
  ...
  let typ_il = elab_plaintyp ctx plaintyp in
  if
    Types.Sub.sub_typ ctx.tdenv typ_il_exp typ_il
    || Types.Sub.sub_typ ctx.tdenv typ_il typ_il_exp
  then
...
and cast_exp (ctx : Ctx.t) (typ_il_expect : Il.typ) (typ_il_infer : Il.typ)
  ...
  if Types.Equiv.equiv_typ ctx.tdenv typ_il_expect typ_il_infer then Ok exp_il
  else if Types.Sub.sub_typ ctx.tdenv typ_il_infer typ_il_expect then
    let exp_il =
...
and elab_arg ?(as_def = false) (ctx : Ctx.t) (param_il : Il.param) (arg : arg) :
  ...
  | DefP (id_p, tparams_il_p, params_il_p, typ_il_p), DefA id_a ->
      ...
      in
      check
        (Types.Equiv.equiv_functyp ctx.tdenv arg.at tparams_il_p params_il_p
           typ_il_p tparams_il_a params_il_a typ_il_a)
        arg.at
        (F.asprintf
           "function argument does not match the declared function parameter %s"
           (Id.to_string id_p));
