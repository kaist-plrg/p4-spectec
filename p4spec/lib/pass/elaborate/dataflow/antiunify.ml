...
let antiunify (dctx : Dctx.t) (exps_group : exp list list) :
    Dctx.t * exp list * prem list list =
  let tdenv = dctx.tdenv in
  let menv = dctx.menv in
  ...
  in
  let dctx = { dctx with frees = IdSet.union dctx.frees unifiers } in
  (dctx, exps_template, prems_match_group)
