...
let compile_extern_func (ctx : Ctx.t) (reverse_dispatch : reverse_dispatch)
  ...
  (* Lookup the original name and type arguments *)
  ...
  let vars_marshal_ml, exprs_marshal_ml =
    ...
    |> List.split
  in
  (* Compile arguments to call the extern function *)
  let exprs_arg_ml =
    Ml.ListE (List.init n (fun i -> Ml.VarE ("v__" ^ string_of_int i)))
  in
  (* Compile type arguments to call the extern function *)
...
let compile_builtin_func (ctx : Ctx.t) (reverse_dispatch : reverse_dispatch)
  ...
  let vars_marshal_ml, exprs_marshal_ml =
    ...
    |> List.split
  in
  (* Compile arguments to call the builtin function *)
  let exprs_arg_ml =
    Ml.ListE (List.init n (fun i -> Ml.VarE ("v__" ^ string_of_int i)))
  in
  (* Compile type arguments to call the builtin function *)
  let exprs_targ_ml = Ml.ListE (List.map Interface.typ_make_expr targs) in
