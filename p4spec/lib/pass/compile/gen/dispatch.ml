...
(* Native dispatchers: [eval_func_native]/[eval_rel_native]
   ...
   the compiled ML backend, some callers live entirely in the typed-[Obj.t]
   world and never held a real [Value.t] to begin with — converting through
   one would be converting from nothing.
   ...
   the generated externs — no witnesses exist here to support a still-generic
   callee, so one is simply not reachable through this entry point; skipped,
   not miscompiled). *)
