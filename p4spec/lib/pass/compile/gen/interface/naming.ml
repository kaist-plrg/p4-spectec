...
let rec mentions (tparams : string list) (typ : Sl.typ) : bool =
  match typ.it with
  | Il.BoolT | Il.NumT _ | Il.TextT | Il.FuncT _ -> false
  | Il.VarT (id, targs) ->
      List.mem id.it tparams || List.exists (mentions tparams) targs
  | Il.TupleT typs -> List.exists (mentions tparams) typs
  | Il.IterT (t, _) -> mentions tparams t
