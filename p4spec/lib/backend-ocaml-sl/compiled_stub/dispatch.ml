(* Stub Dispatch for the [spec_parts_sl] library — copied into [compiled/] by
   `make restore-stub`. Every entry point fails, telling the user to generate
   the real interpreter. Overwritten by `make gen-ocaml-sl`. *)
...
let eval_program (_relname : string) (_includes : string list) (_path : string)
  ...
  Fail
    (`Runtime
       (no_region, "ML interpreter: run `make gen-ocaml-sl` to generate it"))
