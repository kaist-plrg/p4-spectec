...
module Make (Interface : Run.INTERFACE) (Extern : Run.EXTERN) () :
  ...
  let eval_func_native name__ typs__ args__ =
    with_trampoline trampoline (fun () ->
        Spec_parts_sl.Dispatch.eval_func_native name__ typs__ args__)

  let eval_rel_native name__ args__ =
    with_trampoline trampoline (fun () ->
        Spec_parts_sl.Dispatch.eval_rel_native name__ args__)
  ...
  let unmarshal_program value =
    with_trampoline trampoline (fun () ->
        Spec_parts_sl.Dispatch.unmarshal_program value)
end
