...
let bits_to_int_unsigned (ctx : Ctx.t) (at : region) (targs : targ list)
    (values_input : value list) : value =
  Extract.zero at targs;
  let value_bits = Extract.one at values_input in
  let bits = bits_of_value value_bits in
  bits_to_int_unsigned' bits |> value_of_bigint ctx
...
let bits_to_int_signed (ctx : Ctx.t) (at : region) (targs : targ list)
    (values_input : value list) : value =
  Extract.zero at targs;
  let value_bits = Extract.one at values_input in
  let bits = bits_of_value value_bits in
