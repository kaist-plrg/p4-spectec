...
let gen_n_p4_il ~(depth : int) =
  ...
  let (module Runner_intermediate), runners_intermediate =
    List.fold_left
      (fun ((module Runner_before : RUNNER), runners) _ ->
        (* later add per-layer config for IL/SL *)
        let module Interface_SpecTec = Interface.SpecTec_IL in
        let runner =
          (module Runner.Make.Make_nonrec
                    (Interface_SpecTec)
                    (Spectec.Make_parametric (Runner_before) (Interface_SpecTec))
                    (Interp_il.Interp.Make)
                    (Interp_sl.Interp.Make) : RUNNER)
        in
  ...
  let boot =
    ...
    (module Runner.Make.Make_nonrec
              (Interface_SpecTec)
              (Spectec.Make_parametric (Runner_intermediate) (Interface_SpecTec))
              (Interp_il.Interp.Make)
              (Interp_sl.Interp.Make) : RUNNER)
  in
  ((module Runner_P4 : RUNNER), runners_intermediate, boot)

let gen_n_p4_sl ~(depth : int) =
  let (module Runner_P4) = (module P4.Make () : RUNNER) in
  let (module Runner_intermediate), runners_intermediate =
    List.fold_left
      (fun ((module Runner_before : RUNNER), runners) _ ->
...
let gen_n_spectec_il ~(depth : int) =
  ...
  let boot =
    ...
    (module Runner.Make.Make_nonrec
              (Interface_SpecTec)
              (Spectec.Make_parametric (Runner_SpecTec_mid) (Interface_SpecTec))
              (Interp_il.Interp.Make)
              (Interp_sl.Interp.Make) : RUNNER)
  in
  ((module Runner_SpecTec_mid : RUNNER), runners_intermediate, boot)

let gen_n_spectec_sl ~(depth : int) =
  let (module Runner_SpecTec_pgm) =
    let module Interface_SpecTec = Interface.SpecTec_SL in
    (module Runner.Make.Make_nonrec (Interface_SpecTec) (Spectec.Make_null)
              (Interp_il.Interp.Make)
