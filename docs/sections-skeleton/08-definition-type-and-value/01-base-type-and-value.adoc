...
In P4~IR~, base types are subdivided into primitive types and number types.
Values are subdivided into primitive and number values, accordingly.

${syntax: baseTypeIR
          baseValue}

[#sec-def-primitiveTypeIR]
[#sec-def-primitiveValue]
...
* The `void` type, which has no values and can be used only in a few
  restricted circumstances. There is no value of type `void`.
...
The typing rules for the integer types are chosen according to the following
principles:
...
* *No undefined behaviors*: P4 attempts to avoid many of C's
  ...
  etc.). P4 computations on integer types have no undefined behaviors.
...
described in <<sec-varint-ops>>.  The following example shows three constant
definitions whose values are arbitrary-precision integers.
