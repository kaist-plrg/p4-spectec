...
* `key`: An expression that describes how the key used for look-up
  is computed.
-0* `actions`: A list of all actions that may be found in the table.
...
// Added
A ``table`` declaration is both a constructor and an instance declaration,
represented as:

[[tableObjectConstructorDef]]
[[tableObject]]
...
[discrete]
==== Well-formedness of table object constructor types
